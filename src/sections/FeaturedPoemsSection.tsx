    ...
    const ctx = gsap.context(() => {
      const scrollTl = gsap.timeline({
        scrollTrigger: {
          trigger: section,
          start: 'top top',
          end: '+=200%', // Increased to make sections stay longer
          pin: true,
          scrub: 0.5,
        },
      });

      // ENTRANCE (0-20%) - gentler distances
      scrollTl.fromTo(
      ...
      scrollTl.fromTo(
        cardsRef.current[0],
        { x: '-25vw', y: '5vh', rotation: -2, opacity: 0 },
      ...
      // STAY VISIBLE LONGER (20% - 85%)

      // EXIT (85-100%) - delayed exit
      scrollTl.fromTo(
        cardsRef.current,
        { y: 0, opacity: 1 },
      ...
      scrollTl.fromTo(
        ...
        { x: 0, opacity: 1 },
        { x: '-8vw', opacity: 0, ease: 'power1.in' },
        0.85
      );
    }, section);
